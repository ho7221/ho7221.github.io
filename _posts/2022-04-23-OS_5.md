...
### Private queue
각 process의 queue간에 load balancing이 필요하다. 이를 위한 balancing algorithm이 필요하다. 이때 push migration, pull migration이 사용되는데 주기적으로 push, pull migration을 통해 load balancing을 한다. 하지만 이때 processor affinity라는 개념이 등장하는데 process간에 이동에서 cache교체비용으로 인해 시간이 소요되기 때문에 한 processor에서 계속 실행되는 것을 선호한다. 이를 processor affinity라고 한다.  
